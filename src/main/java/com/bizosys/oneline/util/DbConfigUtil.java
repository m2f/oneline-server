...
public class DbConfigUtil {
	...
	public static final void initializePool(Object[] poolUrlUserPass) {
		...
		config.setIsolationLevel(4);
		config.setIncrementBy(2);
		config.setIdleConnections(2);
		config.setDefaultPool(false);
		config.setPoolName(poolUrlUserPass[0].toString());
		config.setConnectionUrl(poolUrlUserPass[1].toString() + OnelineServerConstants.CONNECTION_URL_OPTION);
		config.setLogin(poolUrlUserPass[2].toString());  
		config.setPassword(poolUrlUserPass[3].toString());
		config.setDriverClass(OnelineServerConstants.JDBC_DRIVER);

		if(LOG.isDebugEnabled()) LOG.debug("Initializing pool for [ " + config.getPoolName() + " ]");
		
		try {
			PoolFactory factory = PoolFactory.getInstance();
			if(factory.contains(config.getPoolName())) {
				factory.stop(true, config.getPoolName());
			}
			factory.setup(config);
		} catch (Exception e) {
			LOG.warn("Error initializing pool for url - " + config.getConnectionUrl());
		}
	}
	...
	public static DbConfig[] getUsers(String machineIp) throws SQLException {
		...
		for (Object[] cols : vals) {
			DbConfig config = new DbConfig();
			config.setLogin(cols[0].toString());
			config.setPassword(cols[1].toString());
